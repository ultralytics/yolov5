...
def attempt_import_wandb():
    try:
        import wandb
        assert hasattr(wandb,
                       '__version__')  # verify package import not local dir
...
class Loggers():
    ...
    def __init__(self,
        ...
        self.best_keys = [
            'best/epoch', 'best/precision', 'best/recall', 'best/mAP_0.5',
            'best/mAP_0.5:0.95'
        ]
        for k in LOGGERS:
        ...
        if wandb and 'wandb' in self.include:
            ...
            if pkg.parse_version(
                    wandb.__version__) >= pkg.parse_version('0.12.11'):
                self.logger.warning(
