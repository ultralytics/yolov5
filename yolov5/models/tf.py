...
class TFPad(keras.layers.Layer):
    ...
    def __init__(self, pad):
        ...
        if isinstance(pad, int):
            ...
            self.pad = tf.constant([[0, 0], [pad[0], pad[0]], [pad[1], pad[1]],
                                    [0, 0]])
...
class TFDWConvTranspose2d(keras.layers.Layer):
    ...
    def call(self, inputs):
        return tf.concat(
            [m(x) for m, x in zip(self.conv, tf.split(inputs, self.c1, 3))],
            3)[:, 1:-1, 1:-1]
    ...
    def call(self, inputs):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                anchor_grid = tf.transpose(self.anchor_grid[i],
                                           [0, 2, 1, 3]) * 4
        ...
        return tf.transpose(x, [0, 2, 1, 3]) if self.training else (tf.concat(
            z, 1), x)
...
def parse_model(d, ch, model, imgsz):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m,
        ...
        if m in [
                nn.Conv2d, Conv, DWConv, DWConvTranspose2d, Bottleneck, SPP,
                SPPF, MixConv2d, Focus, CrossConv, BottleneckCSP, C3, C3x
        ]:
            c1, c2 = ch[f], args[0]
