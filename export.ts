...
sys.path.append(FILE.parents[0].as_posix())  # add yolov5/ to path
 ...
 utils.activations  Hardswish, SiLU
 utils.general  colorstr, check_img_size, check_requirements, file_size, set_logging
  ...
  export_torchscript(model, img, file, optimize)
    ...
    prefix  colorstr('TorchScript:')
       :
        ...
        (optimize_for_mobile(ts)  optimize   ts).save(f)
             (f'{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
              ts
         Exception   e:
             (f'{prefix} export failure: {e}')
