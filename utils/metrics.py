...
def ap_per_class(
    ...
    write_f1_csv(px, f1, Path(save_dir) / f"{prefix}F1.csv", names)
...
# Write ----------------------------------------------------------------------------------------------------------------
@threaded
def write_f1_csv(px, py, save_dir=Path("f1.csv"), names=[]):
    # Write F1 curve values to file
    with open(save_dir, "w") as csv_file:
        wr = csv.writer(csv_file, quoting=csv.QUOTE_NONNUMERIC)
        headers = ["Confidence", "mean F1"]
        headers.extend(names)
        wr.writerow(headers)
        mean_f1 = [mean(x) for x in zip(*py)]
        f1_classes = [mean_f1]
        f1_classes.extend(py)
        data = [px]
        data.extend(f1_classes)
        zip_l = zip(*data)

        cleaned_data = clean_f1_data(zip_l)
        wr.writerows(cleaned_data)


def clean_f1_data(data):
    new_data = []
    conf_seen = set()
    for row in data:
        conf = round(row[0], 2)
        if conf not in conf_seen:
            new_row = [round(x, 2) for x in row]
            new_data.append(new_row)
            conf_seen.add(new_row[0])
    return new_data
