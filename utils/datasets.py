...
#
# class LoadImagesFromDB:
#     def __init__(self, img_urls, logger, img_size=640):
#         for idx, (p_id, img_url) in enumerate(img_urls):
#
#             logger.info(
#                 " [SYS-OCR] # Processing [{}] {} ({:d}/{:d})".format(p_id, img_url, (idx + 1), len(img_urls)))
#             dir_name, core_name, ext = utils.split_fname(img_url)
#             rst_path = args.out_path
#             this.time_arr = [time.time()]
#
#             img = url_to_image(img_url, color_fmt='TRANS')  # transparency
#
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        if self.rect:
            ...
            self.labels_files = [self.label_files[i] for i in irect]
