...
class WandbLogger:
                ...
                if isinstance(opt.data, dict):
                    ...
                    if isinstance(opt.resume, str) and opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
                        self.data_dict = {
                            ...
                            for k, v in self.wandb_run.config.items() if k.startswith("data_dict_")}
                    else:  # local resume
    ...
    def log_dataset_artifact(
        ...
        if not log_val_only:
            ...
            if data.get("train"):
                data["train"] = (WANDB_ARTIFACT_PREFIX + str(Path(entity) / Path(project) / "train") + ":" + alias)
        ...
        if data.get("val"):
            data["val"] = (WANDB_ARTIFACT_PREFIX + str(Path(entity) / Path(project) / "val") + ":" + alias)
    ...
    def create_dataset_table(
        ...
        for si, (img, labels, paths, shapes) in enumerate(tqdm(dataset)):
            ...
            boxes = {
                "ground_truth": {
                    "box_data": box_data,
                    "class_labels": {k: str(v)
                                     for k, v in class_to_id.items()},}}  # inference-space
            table.add_data(
                si,
