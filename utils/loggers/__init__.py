...
class Loggers:
    ...
    def remote_dataset(self):
        """Fetches dataset dictionary from remote logging services like ClearML or Comet ML."""
        data_dict = None
        if self.clearml:
    ...
    def on_val_end(self, nt, tp, fp, p, r, f1, ap, ap50, ap_class, confusion_matrix):
        """Logs validation results to ClearML and Comet ML at the end of the validation process."""
        if self.clearml:
    ...
    def on_model_save(self, last, epoch, final_epoch, best_fitness, fi):
        """Callback that handles model saving events, logging to ClearML if enabled."""
    ...
    def on_params_update(self, params: dict):
        """Updates experiment hyperparameters or configurations in Comet ML or ClearML."""
        if self.comet_logger:
...
class GenericLogger:
    ...
    def __init__(self, opt, console_logger, include=("tb", "clearml")):
    ...
    def update_params(self, params):
        """Updates logged parameters in ClearML if enabled."""
        if self.clearml:
            self.clearml.task.connect(params)
