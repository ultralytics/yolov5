...
def process_mask_upsample(proto_out, out_masks, bboxes, shape):
    ...
    masks = (out_masks @ proto_out.float().view(c, -1)).sigmoid().view(-1, mh, mw)
    masks = F.interpolate(masks[None], shape, mode='bilinear', align_corners=False)[0]  # CHW # out_masks.tanh()
...
def process_mask(proto_out, out_masks, bboxes, shape, upsample=False):
    ...
    c, mh, mw = proto_out.shape  # CHW
    ih, iw = shape
    masks = (out_masks @ proto_out.float().view(c, -1)).sigmoid().view(-1, mh, mw)  # CHW  # tanh
