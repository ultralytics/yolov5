...
def _create(name, pretrained=True, channels=3, classes=80, autoshape=True, verbose=True, device=None):
    """```python."""
    Create or load a YOLOv5 model with optional customizations and pretrained weights.
    
    Args:
        name (str): Model name (e.g., 'yolov5s') or the path to the model checkpoint (e.g., 'path/to/best.pt').
        pretrained (bool, optional): Load pretrained weights if True. Defaults to True.
        channels (int, optional): Number of input channels expected by the model. Defaults to 3.
        classes (int, optional): Number of classes the model should detect. Defaults to 80.
        autoshape (bool, optional): Apply the YOLOv5 .autoshape() wrapper if True for diverse input formats. Defaults to True.
        verbose (bool, optional): Print detailed information during model creation/loading if True. Defaults to True.
        device (str | torch.device | None, optional): Device to use for the model (e.g., 'cpu', 'cuda'). If None, selects 
            the best available device. Defaults to None.
    
    Returns:
        DetectMultiBackend | AutoShape: The loaded YOLOv5 model, optionally wrapped with AutoShape if specified.
    ...
    Examples:
        ...
        # Load a model with specific input channels and number of classes
...
def yolov5s(pretrained=True, channels=3, classes=80, autoshape=True, _verbose=True, device=None):
    ...
    Args:
        ...
        device (str | torch.device | None, optional): Device to use for model computation, can be 'cpu', 'cuda', or left
            as None for automatic device selection. Defaults to None.

    Returns:
        torch.nn.Module: The YOLOv5-small model instance, configured with specified parameters and optionally loaded with
            pretrained weights.
    ...
    Notes:
        For more details on model loading and customization, refer to the official documentation at:
...
def yolov5x(pretrained=True, channels=3, classes=80, autoshape=True, _verbose=True, device=None):
    ...
    Args:
        prediction (torch.Tensor): YOLO model inference prediction boxes and scores, with shape (N, 6) for
            (x1, y1, x2, y2, score, class).
        conf_thres (float): Confidence threshold for filtering predictions.
        iou_thres (float): IoU threshold for NMS.
        classes (list | None): Filter by class, i.e. (0, 1, 2).
        agnostic (bool): Class-agnostic NMS.
        multi_label (bool): Apply NMS across all classes (multi-label).
        labels (list | None): List of labels for NMS.

    Returns:
        (list[torch.Tensor]): List of detections, each a torch Tensor of shape (N, 6) where n is the number of detections
            after NMS.

    Example:
        ```python
        prediction = torch.rand(100, 6)  # 100 predictions
        results = non_max_suppression(prediction, 0.5, 0.45)
        ```

    Note:
        If `masks` is True, the function will compute IoU between predicted and ground truth masks.
    """
