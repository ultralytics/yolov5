...
from Processor_dynamic import Processor
...
def parse_args():
    ...
    parser.add_argument('--infer-shape', nargs='+', type=int, default=[8, 3, 640, 640],
        help='if build engine with dynamic shape, you must specify the dimension during inference')
    parser.add_argument(
...
def generate_results(processor, imgs_dir, jpgs, results_file, conf_thres, iou_thres, non_coco, infer_shape):
    ...
    for jpg in jpgs:
        ...
        if batch_id < infer_shape[0]:
            ...
            batch_id += 1
            if batch_id == infer_shape[0]:
                output = processor.detect(img_list)
                pred = processor.post_process(output, image_shape_list, conf_thres=conf_thres,
                                                        iou_thres=iou_thres)
                for index, item in enumerate(pred):
                    for p in item.tolist():
                        x = float(p[0])
                        y = float(p[1])
                        w = float(p[2] - p[0])
                        h = float(p[3] - p[1])
                        results.append({'image_id': image_id_list[index],
...
def main():
    ...
    # setup processor
    processor = Processor(model=args.model, letter_box=True, infer_shape=args.infer_shape)
    ...
    generate_results(processor, args.imgs_dir, jpgs, results_file, args.conf_thres, args.iou_thres,
                     non_coco=False, infer_shape=args.infer_shape)
