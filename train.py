...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    names = ["item"] if opt.single_cls else list(data_dict["names"][0].split(","))
    nc = 1 if opt.single_cls else (int(data_dict["nc"]))  # number classes, names
    ...
    if cuda and rank != -1:
        model = DDP(
            model, device_ids=[int(opt.device)], output_device=[int(opt.device)]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data",
        type=str,
        default="./data/graph_area/graph_table.yaml",
        help="data.yaml path",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--multi_scale", action="store_true", help="vary img-size +/- 50%%"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--name", default="graph_table_yolov5s_results", help="save to project/name"
    )
    parser.add_argument(
        "--exist_ok",
        action="store_true",
    ...
    ## Additional CUDA Infos
    if device.type == "cuda":
        print(torch.cuda.get_device_name(int(GPU_NUM)))
        print("Memory Usage:")
        print(
            "Allocated:",
            round(torch.cuda.memory_allocated(int(GPU_NUM)) / 1024 ** 3, 1),
            "GB",
        )
        print(
            "Cached:   ",
            round(torch.cuda.memory_cached(int(GPU_NUM)) / 1024 ** 3, 1),
            "GB",
        )
