...
from utils.armadillo_dataloaders import exif_transpose, letterbox
from utils.armadillo_general import (
    LOGGER,
    ROOT,
    Profile,
    check_requirements,
...
from utils.armadillo_torch_utils import copy_attr, smart_inference_mode
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        from models.armadillo_experimental import attempt_download, attempt_load  # scoped to avoid circular import
        ...
        if pt:  # PyTorch
            ...
            if edgetpu:  # TF Edge TPU https://coral.ai/software/#edgetpu-runtime
                ...
                vx_delegate = load_delegate("libvx_delegate.so")
                interpreter = Interpreter(model_path=w, experimental_delegates = [vx_delegate])
            interpreter.allocate_tensors()  # allocate
            input_details = interpreter.get_input_details()  # inputs
            output_details = interpreter.get_output_details()  # outputs
            # TFLiteが入力を期待する画像の仕様
            #LOGGER.info(f"TFLite model input_details: {input_details[0]}")
            # load metadata
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            ...
            im = im.contiguous().cpu().numpy()  #処理の高速化
    ...
    def _model_type(p="path/to/model.pt"):
        ...
        from armadillo_export import export_formats
