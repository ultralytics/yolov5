...
class DFL(nn.Module):
    ...
    def __init__(self, c1=16):
        ...
        x = torch.arange(c1, dtype=torch.float).softmax(0)
        self.conv.weight.data[:] = nn.Parameter(x.view(1, c1, 1, 1))
        self.c1 = c1

    def forward(self, x):
        b, c, a = x.shape  # batch, channels, anchors
        return self.conv(x.view(b, 4, self.c1, a).transpose(2, 1)).view(b, 4, a)  # no-soft
        # return self.conv(x.view(b, 4, self.c1, a).transpose(2, 1).softmax(1)).view(b, 4, a)  # soft
