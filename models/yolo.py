...
class DetectSplit(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.inplace = inplace  # use inplace ops (e.g. slice assignment)
        self.shape = (0, 0)  # initial grid shape
        self.cv2 = nn.ModuleList(nn.Sequential(Conv(x, x, 3), nn.Conv2d(x, 4, 1, padding=0)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, x, 3), nn.Conv2d(x, self.no - 4, 1, padding=0)) for x in ch)
