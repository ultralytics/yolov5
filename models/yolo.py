...
class DetectSplit(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # self.cv1 = nn.ModuleList(nn.Sequential(Conv(x, x, 1), Conv(x, x, 1)) for x in ch)
        self.cv2 = nn.ModuleList(nn.Sequential(nn.Conv2d(x, 5, 3, padding=1), ) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(nn.Conv2d(x, self.no - 5, 1, padding=0), ) for x in ch)
