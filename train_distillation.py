...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 7) % ('Epoch', 'box', 'obj', 'cls', 'd_loss', 's_loss', 'img_size'))
        if RANK in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(('%10s' * 1 + '%10.4g' * 6) % (
                    f'{epoch}/{epochs - 1}', *mloss, d_outputs_loss, s_loss, imgsz))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--t_cfg', type=str, default='', help='teacher model.yaml path')
