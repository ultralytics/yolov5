...
if not firebase_admin._apps:
    ...
    firebase_admin.initialize_app(cred, {
        'databaseURL': 'https://animal-70086-default-rtdb.europe-west1.firebasedatabase.app/'
    })
...
if SEND_TO_USERS_WITHOUT_LOCATION:
    print(f"⚠️ وضع التجربة: سيتم الإرسال للمستخدمين بدون موقع أيضاً")


# دالة حساب المسافة بين نقطتين جغرافيتين - تستخدم معادلة Haversine
def calculate_distance(lat1, lon1, lat2, lon2):
    """
    حساب المسافة بين إحداثيين جغرافيين باستخدام معادلة Haversine
    الإرجاع: المسافة بالكيلومتر
    """
...
def broadcast_animal_alert(animal_type, distance_from_cam):
    ...
    try:
        ...
        print(f"🔍 البحث عن المستخدمين القريبين...")
        ...
        for doc in fs.collection('users').stream():
            ...
            if user_lat is None or user_lng is None:
                ...
                if SEND_TO_USERS_WITHOUT_LOCATION:
                    ...
                    print(f"    → تم إضافة المستخدم (وضع التجربة)")
                continue
        ...
        if nearby_tokens:
            ...
            print(f"💡 نصيحة: تأكد من أن المستخدمين لديهم 'latitude' و 'longitude' في Firestore.")
