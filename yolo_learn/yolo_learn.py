...
def main_generate(ini, logger=None):
    ...
    for idx, raw_fname in enumerate(raw_fnames):
        ...
        try:
            ...
            # Extract crop position
            object_names = ini["object_names"].replace(" ", "").split("_")
            ...
            for obj in objects:
                ...
                elif object_type == "graph_table":

                    if object_name == "graph":
                        class_num = 0
...
def main_train_test(ini, logger=None):
    ...
    subprocess.call(train_cmd, cwd=ini["train_root_path"])  # train by python
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "--target_mode",
        required=True,
        choices=["PROBLEM", "GRAPH", "GRAPH_TABLE"],
        help="Target mode",
    )
    parser.add_argument(
        "--op_mode",
...
SELF_TEST_ = True
TARGET = "GRAPH_TABLE"  # PROBLEM / GRAPH
...
elif TARGET == "GRAPH_TABLE":
    INI_FNAME = _this_basename_ + "_graph_table.ini"
