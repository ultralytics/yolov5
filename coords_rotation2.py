            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if cls == 4 : 
                        ...
                        vertices = np.array([[tlx_a, tly_a], [brx_a, tly_a], [brx_a, bry_a], [tlx_a, bry_a]], dtype=np.int32)
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if cls == 6 and n_pedunculo == 0 and n_calice == 0: 
                                ...
                                distance_c = math.dist((center_point_apple[0], center_point_apple[1]), (center_point_c[0], center_point_c[1]))
                                
                                distancia = [center_point_apple[0] - center_point_c[0] , center_point_apple[1] - center_point_c[1]]
                                distancia_1 = [center_point_apple[0] + distancia[0] , center_point_apple[1] + distancia[1]]
                                ...
                                if distance_c > 0:
                                    calice_angle = math.acos(round((((tlx_c+brx_c)/2)-((tlx_a+brx_a)/2)))/distance_c) * (180/math.pi)
                                ...
                                if center_point_apple[0] != 0 and center_point_apple[1] != 0:
                                    try:
                                        ...
                                        vertices_second_half = np.array([[int(object_line[0,0]), int(object_line[0,1])],
                                                            [int(object_line[1,0]), int(object_line[1,1])],
                                                            [int(brx_a), int(tly_a)],
                                                            [int(brx_a), int(bry_a)]])


                                        vertices_first_half = np.array([[int(object_line[0,0]), int(object_line[0,1])],
                                                            [int(object_line[1,0]), int(object_line[1,1])],
                                                            [int(tlx_a), int(tly_a)],
                                                            [int(tlx_a), int(bry_a)]])
                                        ...
                                        calice_angle_y = math.acos(round((((tly_c+bry_c)/2)-((tly_a+bry_a)/2)))/distance_c) * (180/math.pi)
                                        
                                        calice_angle_x = math.acos(round((((tlx_c+brx_c)/2)-((tlx_a+brx_a)/2)))/distance_c) * (180/math.pi)
                                        ...
                                        R = np.eye(3) * cos_theta + (1 - cos_theta) * np.outer(axis, axis) + sin_theta * K
                                        ...
                                        resized_img = cv2.resize(rotated_img, (int(w / scale_factor), int(h / scale_factor)))
                                        ...
                                        cv2.imwrite(f"/home/gabriel/projeto/yolov5/runs/detect/calice/{filename1}", cropped_img)
                            ...
                            if cls == 5 and n_calice == 0 and n_pedunculo == 0:
                                ...
                                distance_p = math.dist((center_point_apple[0], center_point_apple[1]), (center_point_p[0], center_point_p[1]))
                                #worksheet.cell(row=row_index, column=4, value=distance)
                                #print("Distance between center point of pedunculo and origin: ", distance_p)
                                print("Distance Pedunculo", distance_p)


                                distancia = [center_point_apple[0] - center_point_p[0] , center_point_apple[1] - center_point_p[1]]
                                distancia_1 = [center_point_apple[0] + distancia[0] , center_point_apple[1] + distancia[1]]
                                ...
                                line_dir = np.array([center_point_p[0] - center_point_apple[0], center_point_p[1] - center_point_apple[1]])
                                ...
                                if distance_p > 0 :                 
                                    try:
                                        pedunculo_angle = math.acos(round((((tlx_p+brx_p)/2)-((tlx_a+brx_a)/2)))/distance_p) * (180/math.pi)
                                ...
                                if diff_pedunculo_angle > 0 :
                                    print("Apple rotated: ",round(diff_pedunculo_angle,1)," degrees about z-axis (cw)")
                                elif diff_pedunculo_angle < 0:
                                    print("Apple rotated: ",round(diff_pedunculo_angle,1)," degrees about z-axis (ccw)")
                                else:
                                    print("Apple didn't rotate about z-axis")
                                ...
                                if center_x_apple != 0 and center_y_apple != 0:
                                    try:
                                        ...
                                        pedunculo_angle_y = math.acos(round((((tly_a+bry_a)/2))-((tly_p+bry_p)/2))/distance_p) * (180/math.pi)
                                        ...
                                        # Resize the rotated image to fill the 320x320 image
                                        resized_img = cv2.resize(cropped_img, (int(300 / scale_factor), int(3000 / scale_factor)))
                                        ...
                                        if apple1 > 0:
                                            ...
                                            cv2.imwrite(f"/home/gabriel/projeto/yolov5/runs/detect/pedunculo/1/{filename1}", result)
                                        ...
                                        if prev_img is not None:
                                            ...
                                            sift = cv2.SIFT_create(nfeatures=1000, contrastThreshold=0.023, sigma=0.9 ,edgeThreshold=edge_threshold, nOctaveLayers=9)
                                            ...
                                            imagem1 = cv2.drawKeypoints(prev_img, kp1, None, color=(0, 255, 0), flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                                            #cv2.imshow("Image Kp",imagem1)
                                            #cv2.waitKey(0)
                                            # Create matcher object
                                            matcher = cv2.FlannBasedMatcher(dict(algorithm=255, table_number=6, key_size=12, crossCheck=True, multi_probe_lever =1))
                                            ...
                                            try:
                                                ...
                                                timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
                                                imMatches = cv2.drawMatches(prev_img, kp1, result, kp2, matches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                                                filenameM1  = f"matchesM1-{timestamp}.jpg"
                                                cv2.imwrite(f"/home/gabriel/projeto/yolov5/runs/detect/matches/{filenameM1}", imMatches)
                                                prev_img = result
